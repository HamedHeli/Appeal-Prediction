...
PCC <- round(sum(predict(weighted_rf_default, 
        newdata = data_test, type = "prob")[, "positive"]))

actual_count <- sum(data_test$Class == 'positive')

```
Actual Prevalence in Test Set is `r actual_count` and the predicted PCC from weighted RF is `r PCC`.
...
cm_weighted_rf_training_tbl <- evaluate_model(weighted_rf_default, 
               data_valid,
               data_train)$cm$table
...
Actual Prevalence in Test Set is `r actual_count` and the predicted ACC from weighted RF is `r ACC`.
